...
import { getSkinConcerns, createSkinConcern, updateSkinConcern, deleteSkinConcern } from "../actions";
import { Plus, Trash2, Loader2, ArrowLeft, X } from "lucide-react";
import { supabase } from "@/lib/supabase";
import { ConfirmModal } from "@/components/admin/ConfirmModal";

export default function AdminSkinConcernsPage() {
  ...
  const [formData, setFormData] = useState<any>({ title: '', slug: '', description: '' });
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const data = await getSkinConcerns();
  ...
  const requestDelete = (id: string) => {
    setModalConfig({
      ...
      message: "Are you sure you want to delete this skin concern?",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await updateSkinConcern(editingItem.id, formData);
      } else {
        await createSkinConcern(formData);
      }
      const data = await getSkinConcerns();
      ...
      <div className="max-w-3xl mx-auto space-y-6 pb-20 p-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl font-serif text-stone-900 dark:text-white">{editingItem ? "Edit" : "New"} Skin Concern</h2>
          </div>
    ...
    <div className="p-8 pb-32">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-serif text-stone-900 dark:text-white">Skin Concerns</h1>
          <button onClick={openCreateView} className="bg-stone-900 dark:bg-white text-white dark:text-stone-900 px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 flex items-center gap-2"><Plus size={16} /> New Skin Concern</button>
        </div>
