...
export default function AdminArticlesPage() {
  ...
  const [formData, setFormData] = useState<any>({ title: '', slug: '', category: 'Regimen', content: '', image_url: '', size: 'medium', is_published: true });
  ...
  const openCreateView = () => {
    setEditingItem(null);
    setFormData({ title: '', slug: '', category: 'Regimen', content: '', image_url: '', size: 'medium', is_published: true });
    setCurrentView('edit');
  };
      ...
      <div className="max-w-3xl mx-auto space-y-6 pb-20 p-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl font-serif text-stone-900 dark:text-white">{editingItem ? "Edit" : "New"} Article</h2>
          </div>
        ...
        <div className="bg-white dark:bg-[#111] p-8 border border-stone-200 dark:border-stone-800 rounded-xl space-y-8">
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={formData.title || ""} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full bg-stone-50 dark:bg-[#0a0a0a] border border-stone-200 dark:border-stone-800 rounded-lg px-4 py-3 text-stone-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-stone-500" />
            </div>
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Size (small/medium/large)</label>
              <input type="text" value={formData.size || ""} onChange={e => setFormData({...formData, size: e.target.value})} className="w-full bg-stone-50 dark:bg-[#0a0a0a] border border-stone-200 dark:border-stone-800 rounded-lg px-4 py-3 text-stone-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-stone-500" />
            </div>
            <div className="flex items-center gap-2">
              <input type="checkbox" checked={formData.is_published || false} onChange={e => setFormData({...formData, is_published: e.target.checked})} className="rounded border-stone-300 text-stone-900 focus:ring-stone-900" />
              <label className="text-sm font-medium text-stone-700 dark:text-stone-300">Is Published</label>
            </div>
            <div>
              <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Content</label>
              <textarea value={formData.content || ""} onChange={e => setFormData({...formData, content: e.target.value})} rows={10} className="w-full bg-stone-50 dark:bg-[#0a0a0a] border border-stone-200 dark:border-stone-800 rounded-lg px-4 py-3 text-stone-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-stone-500 font-serif leading-relaxed" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="p-8 pb-32">
      <div className="max-w-5xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-serif text-stone-900 dark:text-white">Articles</h1>
          <button onClick={openCreateView} className="bg-stone-900 dark:bg-white text-white dark:text-stone-900 px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90 flex items-center gap-2"><Plus size={16} /> New Article</button>
        </div>
              ...
              {items.map((item: any) => (
                <div key={item.id} className="p-6 flex items-center justify-between hover:bg-stone-50 dark:hover:bg-stone-900/50 transition-colors">
                  <div className="flex flex-col gap-1">
                    <h3 className="text-sm font-medium text-stone-900 dark:text-white">{item.title}</h3>
